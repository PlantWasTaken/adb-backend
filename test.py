...
class BaseDevice:
    ...
    def get_info(self, device_identifier) -> None:
        ...
        print(f'{device_identifier} App in focus: {self._currentapp}')
        print(f'{device_identifier} Orientation: {self.ORIENTATION}')
    ...
    def app_resolution(self, device_identifier) -> None: #for tsting
        text = subprocess.run(f'"{self.adb}"  -s {device_identifier} shell dumpsys activity top ', shell=True,text=True,capture_output=True)
        print(text.stdout)
        #| find "mCurrentConfig"
    ...
    def currentfocus(self, device_identifier) -> str:
        ...
        return _currentfocus.stdout.strip()
...
class Phone(BaseDevice):
    def __init__(self, name, vertical=True,adb_path=None) -> None:
        ...
        if(self.ORIENTATION == 'ROTATION_90' or self.ORIENTATION == 'ROTATION_270'):
            ...
            self.res_scalar_x = phone_resolution[0]/self.BASE_RESOLUTION_EMU[0] #eg 2400/1920
            self.res_scalar_y = phone_resolution[1]/self.BASE_RESOLUTION_EMU[1]
        else:
            self.res_scalar_x = phone_resolution[1]/self.BASE_RESOLUTION_EMU[0]
            self.res_scalar_y = phone_resolution[0]/self.BASE_RESOLUTION_EMU[1]
    ...
    def app_resolution(self) -> None:
        super().app_resolution(self.name)
...
class Emulator(BaseDevice):
    ...
    def currentfocus(self) -> str:
        identifier = f"emulator-{self.port}" if self.emulator else self.name
        super().currentfocus(identifier)

    def orientation(self) -> str:
        identifier = f"emulator-{self.port}" if self.emulator else self.name
        super().orientation(identifier)

    def app_resolution(self) -> None:
        identifier = f"emulator-{self.port}" if self.emulator else self.name
        super().app_resolution(identifier)
